          ...
          {MOCK_SUCCESS_STORIES.map((story) => (
            ...
            >
              <div className="relative w-full h-56 bg-rose-50">
                <Image
                  src={story.photoUrl}
                  alt={story.coupleNames || `${story.husbandName || 'Couple'} & ${story.wifeName || ''}`}
                  fill
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white">
                  <span className="font-serif font-bold text-lg">
                    {story.coupleNames || `${story.husbandName || ''} & ${story.wifeName || ''}`}
                  </span>
                  <Badge variant="wine" size="sm" className="bg-white/90 text-brand-wine">
                    {story.marriageYear || story.marriageDate || '2026'}
                  </Badge>
                </div>
              </div>
